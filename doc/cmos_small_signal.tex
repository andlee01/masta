...
\newcommand{\nmosmx}[3]{
  ...
  \begin{scope}[#2]
    ...
    % Define input/output ports
    \coordinate (G#1) at (2,1);  % Port G
    \coordinate (D#1) at (0,3);  % Port D
    \coordinate (S#1) at (0,1);  % Port S
...
\newcommand{\pmos}[3]{
  ...
  \begin{scope}[#2]
    ...
    % Define input/output ports
    \coordinate (G#1) at (0,3);  % Port G
    \coordinate (S#1) at (2,3);  % Port S
    \coordinate (D#1) at (2,1);  % Port D
    ...
    % Isd
    \draw (S#1) to[I, l^=$gm_{#3}$] (D#1);
    % Ro
    \draw (S#1) to[short] ++(2,0) to[R, l^=${R_O}_{#3}$] ++(0,-2) to[short] (D#1);
    % Isg
    \draw (S#1) to[I, v_=${v_{sg}}_{#3}$] (G#1);
...
\newcommand{\pmosmx}[3]{
  ...
  \begin{scope}[#2]
    ...
    % Define input/output ports
    \coordinate (G#1) at (2,3);  % Port G
    \coordinate (S#1) at (0,3);  % Port S
    \coordinate (D#1) at (0,1);  % Port D
