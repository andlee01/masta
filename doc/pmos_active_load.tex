...
\begin{document}
 \begin{circuitikz}
    ...
    % The FET transistor
    \draw (0,4) node[pmos, xscale=-1, anchor=S](F){} node[below left,  yshift=0.3cm] at (F) {M1};   
    \draw (4,4) node[pmos, xscale=1,  anchor=S](G){} node[below right, yshift=0.3cm] at (G) {M2};
