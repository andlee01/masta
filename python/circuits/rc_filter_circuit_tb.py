...
def ext_crop_cycles(fig, tr, yr, labels, quantity_types, cycles=2.5, skip_cycles=2, context=None):
...
def plot_tr(tr, yr, filename="plot.html", title="System Response", labels=None, quantity_types=None, extensions=None):
    ...
    tr_p, yr_p = context.get("tr_crop", tr), context.get("yr_crop", yr)
    
    # --- NEW: Calculating symmetrical bounds to align the 0-lines ---
    ...
    # Get absolute maximums with a 10% padding buffer
    v_bound = np.max(np.abs(yr_p[:, v_idxs])) * 1.1 if v_idxs else 1
    i_bound = np.max(np.abs(yr_p[:, i_idxs])) * 1.1 if i_idxs else 1

    fig = go.Figure()
    for i in range(num_signals):
        is_v = quantity_types[i].lower() == "voltage"
        fig.add_trace(go.Scatter(x=tr_p, y=yr_p[:, i], name=labels[i], yaxis="y" if is_v else "y2"))

    # Apply the symmetrical ranges to force the zero lines to match exactly in the center
    fig.update_layout(
        title=title, xaxis_title="Time (s)",
        yaxis=dict(
            title="Voltage (V)", side="left", 
            range=[-v_bound, v_bound],       # Symmetrical Range
            zeroline=True, zerolinecolor="lightgray", zerolinewidth=1 # Emphasize 0 line
        ),
        yaxis2=dict(
            title="Current (A)", side="right", overlaying="y", showgrid=False, 
            range=[-i_bound, i_bound],       # Symmetrical Range
            zeroline=True, zerolinecolor="lightgray", zerolinewidth=1
        ),
        template="plotly_white", legend=dict(orientation="h", y=1.1, x=1)
    )
...
def run_rc_transient(net, omega, num_cycles=NUM_CYCLES, mag=DEFAULT_MAG, tstep_per_cycle=1000):
    """
    ...
    # Input waveform
    v_input = net.get_sine_time_series_input(tr=tr)
    ...
    # Append signals
    yr = np.column_stack((yr, v_input, i_res))

    return tr, yr


def plot_rc_results(tr, yr, omega, prefix="rc_filter_m1", base_path="."):
    """
    ...
    plot_tr(
        tr=tr,
        yr=yr,
        labels=[r"$V_C$", r"$V_{IN}$", r"$i_R$"],
        quantity_types=["voltage", "voltage", "current"],
...
def main():
    ...
    params = {"R": DEFAULT_R, "C": DEFAULT_C, "VIN": 0}

    nw_m0             = rc_filter_ntwrk(lti=True)
    nw_m0.set_rc_filter_params(**params)
    nw_m1             = rc_filter_ntwrk(lti=False, input_type=rc_filter_ntwrk._sine_source)
    nw_m1.set_rc_filter_params(**params)
